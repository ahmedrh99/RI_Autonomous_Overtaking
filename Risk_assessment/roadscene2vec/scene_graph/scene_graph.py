...
class SceneGraph:
    ...
    def __init__(self, relation_extractor, framedict= None, framenum=None, bounding_boxes = None, bev = None, coco_class_names=None, platform='carla'):#, depth_points=None):
        #configure relation extraction settings
        self.relation_extractor = relation_extractor
        
        self.platform = platform
        
        #self.depth_points = depth_points  # <-- store lidar for use in other methods

        self.angle_per_pix = 110/640 
    ...
    def get_nodes_from_bboxes(self, bev, boxes, labels, coco_class_names):
        for idx, (box, label) in enumerate(zip(boxes, labels)):
            ...
            if attr['top'] >= bev.params['height'] - 100: 
                continue
            ...
            #depth = self.get_depth(self.depth_points, x_mid, y_bottom)
            
            #print(f"depth value {depth}")
            
            
            
            #angle = (x_mid - 320)* self.angle_per_pix
            #print(f"angle for detectionn {angle}")
            
            #attr['location_x'] = depth * np.sin(np.radians(angle))
            #attr['location_y'] = depth * np.cos(np.radians(angle))
            
            #print(f"location is at ({attr['location_x']} , {attr['location_y']})")
            
            

            #x_bev, y_bev = self.xy_from_lidar(x_mid, y_bottom)
            #x_refined, y_refined = self.refine_with_lidar(x_bev, y_bev, self.lidar_points)
...
#==========================================================================================
    ...
    def get_depth(self, depth_map, x, y):
        """
        Returns the depth value at pixel (x, y) from the depth map.
        
        Parameters:
            depth_map (np.ndarray): The 2D array representing the depth image.
            x (int): The x (column) coordinate in the image.
            y (int): The y (row) coordinate in the image.
        
        Returns:
            float: The depth value at the specified pixel.
        """
    ...
    def refine_with_lidar(x_bev, y_bev, lidar_points_2d, max_distance=1.0):
        """
        Refine a BEV-projected point (x_bev, y_bev) using nearest LiDAR point.

        Args:
            x_bev, y_bev: Coordinates from BEV projection.
            lidar_points_2d: Nx2 numpy array of [x, y] lidar points in BEV/world frame.
            max_distance: Max distance to consider a valid match.

        Returns:
            Tuple of (x, y) — the refined position from LiDAR, or original point if no close match.
        """

        if lidar_points_2d.shape[0] == 0:
            return x_bev, y_bev

        # Build KDTree from lidar points
        tree = cKDTree(lidar_points_2d)

        # Query the nearest point
        distance, index = tree.query([x_bev, y_bev], k=1)

        if distance <= max_distance:
            return lidar_points_2d[index]
        else:
            # No close lidar point found
            return x_bev, y_bev
