...
class CarlaRosBridge(CompatibleNode):
    ...
    def _synchronous_mode_update(self):
        ...
        while not self.shutdown.is_set() and roscomp.ok():
            ...
            self.logdebug(f"Sync update loop - Frame: {frame}, Shutdown flag: {self.shutdown.is_set()}, ROS OK: {roscomp.ok()}")
...
def main(args=None):
    ...
    parameters['fixed_delta_seconds'] = carla_bridge.get_param('fixed_delta_seconds',
                                                               0.05)
    parameters['register_all_sensors'] = carla_bridge.get_param('register_all_sensors', False)
    parameters['town'] = carla_bridge.get_param('town', '')
    parameters['publish_tf']    = carla_bridge.get_param('publish_tf', False)
    parameters['tf_update_rate'] = carla_bridge.get_param('tf_update_rate', 10.0)
